        ...
        <SimpleGrid columns={{ base: 1, lg: 3, xl: 4 }} spacing={10}>
          {(recipes && recipes.results) &&
            recipes.results.map((recipe) => {
              return (
                <MotionBox key={recipe.id} layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <Box p={4} bg="white" rounded="md" shadow="lg">
                  <Image borderRadius="lg" src={recipe.image} alt={recipe.title} mb={2} />
                  <Text fontWeight="bold" fontSize="lg">{recipe.title}</Text>
                  <Button mt={2} colorScheme="teal" onClick={() => {
